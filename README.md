...
1. Applies `maven-publish` and creates a default `mavenJava`
   `MavenPublication` from the `java` software component (when present).
...
5. Applies a typed `pomDefaults { … }` block (from the `octopusPublishing`
   extension) to every `MavenPublication`'s POM.
...
pluginManagement {
    repositories {
        gradlePluginPortal()
        // or your internal JFrog plugin repo
    }
    plugins {
...
octopusPublishing {
    ...
    pomDefaults {
        ...
        scm("https://github.com/my-org/my-library.git")
        developer("octopus", "Octopus Team")
    }
}
```
...
| --- | --- |
| POM customization (`pomDefaults`) | `OctopusPublishingPluginFT.testPomCustomization` |
...
| Source layout | Monolithic `ReleaseManagementGradlePlugin.groovy` (~300 lines) | Slim Groovy bootstrap + focused Kotlin/Groovy classes (`PublishingConfigurer`, `PomCustomizer`, `ArtifactoryConfigurer`) |
